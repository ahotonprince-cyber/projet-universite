...
app.get('/health/email-test', async (req, res) => {
    ...
    const to = req.query.to || process.env.SMTP_USER || process.env.BREVO_LOGIN;
    const provider = process.env.BREVO_SMTP_KEY ? 'Brevo SMTP' : 'Gmail SMTP';
    const ok = await sendEmail(to, `Test COWEC (${provider})`, `<h2>${provider} fonctionne depuis Railway !</h2><p>Envoyé à : ${to}</p>`);
    if (ok) res.json({ success: true, provider, to });
    else res.status(500).json({ success: false, provider, error: 'Voir logs Railway pour le détail' });
});
