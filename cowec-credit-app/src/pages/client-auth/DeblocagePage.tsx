...
export default function DeblocagePage() {
  ...
  const [status, setStatus] = useState<Status>(token ? 'loading' : 'form');
  ...
  useEffect(() => {
    ...
    fetch(`/api/auth/debloquer?token=${token}`)
      .then(r => r.json())
      .then(data => {
        if (data.success) {
          setStatus('success');
  ...
  const handleDemande = async (e: React.FormEvent) => {
    ...
    setSendMessage('');
    try {
      const res = await fetch('/api/auth/demander-deblocage', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ email }),
      });
      const data = await res.json();
      setSendMessage(data.message || 'Email envoyé.');
    } catch {
      setSendMessage('Erreur lors de l\'envoi. Réessayez.');
    } finally {
      setSending(false);
    }
  };
    ...
    >
      ...
      <div className="relative z-10 w-full max-w-md bg-black/40 backdrop-blur-lg border border-white/15 rounded-2xl p-8 shadow-2xl text-center">
        ...
        <div className="flex items-center justify-center gap-3 mb-8">
          <div className="w-10 h-10 bg-white/30 border border-white/40 rounded-xl flex items-center justify-center">
        ...
        {status === 'loading' && (
          <>
            ...
            <p className="text-white/80">Déblocage en cours...</p>
          </>
        )}
        ...
        {status === 'success' && (
          <>
            <div className="w-16 h-16 bg-green-500/20 border border-green-400/40 rounded-full flex items-center justify-center mx-auto mb-4">
              <i className="ri-lock-unlock-line text-green-300 text-3xl" />
            </div>
            <h2 className="text-white text-2xl font-bold mb-2">Compte débloqué !</h2>
        ...
        {status === 'error' && (
          <>
            <div className="w-16 h-16 bg-red-500/20 border border-red-400/40 rounded-full flex items-center justify-center mx-auto mb-4">
              <i className="ri-lock-line text-red-300 text-3xl" />
            </div>
            <h2 className="text-white text-2xl font-bold mb-2">Lien invalide</h2>
            <p className="text-white/70 text-sm mb-6">{message}</p>
            <p className="text-white/60 text-sm mb-3">Demandez un nouveau lien de déblocage :</p>
            {sendMessage ? (
              <p className="text-green-300 text-sm font-medium">{sendMessage}</p>
            ) : (
              <form onSubmit={handleDemande} className="space-y-3 text-left">
                <input
                  type="email"
                  value={email}
                ...
                >
                  {sending
                    ...
                    : <><i className="ri-mail-send-line" /> Envoyer le lien de déblocage</>}
                </button>
              </form>
            )}
          </>
        ...
        {/* Formulaire direct (pas de token dans l'URL) */}
        {status === 'form' && (
          <>
            <div className="w-16 h-16 bg-orange-500/20 border border-orange-400/40 rounded-full flex items-center justify-center mx-auto mb-4">
              <i className="ri-lock-line text-orange-300 text-3xl" />
            </div>
            <h2 className="text-white text-2xl font-bold mb-2">Débloquer mon compte</h2>
            <p className="text-white/70 text-sm mb-6">
              Entrez votre adresse email pour recevoir un lien de déblocage.
            </p>
            {sendMessage ? (
              <p className="text-green-300 text-sm font-medium">{sendMessage}</p>
            ) : (
